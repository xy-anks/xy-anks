<!-- Banner -->
![banner](https://capsule-render.vercel.app/api?type=waving&color=0:0f4c75,100:3282b8&height=200&section=header&text=Supply%20Chain%20%26%20OR&fontSize=35)

# 👋 Hi, I'm [你的名字] / 你好，我是 [你的名字]

🎓 Logistics Management Student | 物流管理专业学生  
📊 Operations Research / Supply Chain Optimization | 运筹学 / 供应链优化方向  
💻 C / C++ / Python Learner | 编程学习者

---

## 🧠 About Me / 关于我

- 📦 Interested in logistics systems and supply chain optimization  
  📦 对物流系统与供应链优化问题感兴趣  

- 📈 Studying Operations Research (LP / IP / Network Flow)  
  📈 正在学习运筹学（线性规划 / 整数规划 / 网络流）  

- 🧠 Building Python-based optimization models  
  🧠 使用 Python 构建优化模型  

- 🚚 Focused on real-world logistics and decision systems  
  🚚 关注真实物流与决策优化问题  

---

## 🚀 Current Focus / 当前学习方向

- OR-Tools for Vehicle Routing Problem (VRP)  
  车辆路径优化问题（VRP）

- Linear Programming with PuLP  
  线性规划建模（PuLP）

- Integer Programming / Warehouse Location Problem  
  整数规划 / 仓库选址问题  

- Data analysis with Python (Pandas / NumPy)  
  Python数据分析（Pandas / NumPy）

---

## 🧠 Tech Stack / 技术栈

![Python](https://img.shields.io/badge/Python-yellow)
![C](https://img.shields.io/badge/C-blue)
![C++](https://img.shields.io/badge/C++-purple)
![Pandas](https://img.shields.io/badge/Pandas-black)
![NumPy](https://img.shields.io/badge/NumPy-blue)
![OR-Tools](https://img.shields.io/badge/OR--Tools-green)
![Git](https://img.shields.io/badge/Git-black)

---

## 📂 Projects / 项目（持续更新）

| Project 项目 | Type 类型 | Status 状态 |
|--------------|----------|-------------|
| Vehicle Routing Problem | OR Optimization 运筹优化 | Learning 学习中 |
| Warehouse Location Model | Integer Programming 整数规划 | Planning 计划中 |
| Transport Cost Minimization | Linear Programming 线性规划 | Building 开发中 |

---

## 📊 GitHub Stats

![stats](https://github-readme-stats.vercel.app/api?username=你的用户名&show_icons=true&theme=tokyonight)

![langs](https://github-readme-stats.vercel.app/api/top-langs/?username=你的用户名&layout=compact&theme=tokyonight)

---

## 📈 Activity Graph / 活动图

![graph](https://github-readme-activity-graph.vercel.app/graph?username=你的用户名&theme=tokyo-night)

---

## 🔥 Contribution Streak / 连续贡献

![streak](https://streak-stats.demolab.com?user=你的用户名&theme=tokyonight)

---

## 🎯 Goals / 目标

- Master OR-Tools & optimization modeling  
  掌握 OR-Tools 与优化建模  

- Build real logistics optimization projects  
  完成真实物流优化项目  

- Strengthen Python data analysis skills  
  提升 Python 数据分析能力  

- Build a strong GitHub portfolio  
  打造完整 GitHub 作品集  

---

## 📫 Contact / 联系方式

- Email: your_email@example.com  
- GitHub: https://github.com/你的用户名  
